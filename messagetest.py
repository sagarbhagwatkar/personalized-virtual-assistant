...
def create_draft(service, sender, recipient, body):
    # Construct the email message without subject
...
def get_llm_response(form_input, email_sender, email_recipient, email_style, email_recipient_name):
    ...
    template ="""
    Write an descriptive email on the topic: {email_topic} with {style} style.\n\nSender: {sender}\nRecipient: {recipient}.
    """
    ...
    email_body = llm(prompt.format(email_topic=form_input, sender=email_sender, recipient=email_recipient_name, style=email_style))
    print(email_body)

    # Create a draft with the email content
    draft_id = create_draft(service, email_sender, email_recipient, email_body)
    return draft_id
...
if submit:
    st.write(get_llm_response(form_input, email_sender, email_recipient, email_style, email_recipient_name))
