...
def get_llm_response(form_input, email_sender, email_recipient, email_style, email_recipient_name):
  service = get_gmail_service()

  llm = Ollama(model="llama3")

  # Template for building the PROMPT (use recipient name for better personalization)
  template = """
  ...
  **To:** {recipient_name}
  ...
  prompt = PromptTemplate(
      input_variables=["style", "email_topic", "sender", "recipient_name"],
      template=template
  )

  # Generating the response using LLM
  email_body = llm(prompt.format(email_topic=form_input, sender=email_sender, recipient_name=email_recipient_name, style=email_style))
  print(email_body)

  # Create a draft with the email content
  draft_id = create_draft(service, email_sender, email_recipient, email_subject="", body=email_body)  # Don't pass subject here, use empty string
  return draft_id
...
email_subject = st.text_input('Email Subject (Optional)')  # Allow optional subject
